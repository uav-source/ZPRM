"""Dry-run qualification and frozen formal runner for Confirmatory v2.
...
def verify_formal_prerun_artifact_binding(
    ...
    live = verify_v2_prerun_artifact(
        artifact, manifest_path=manifest_path, write_report=False
    )
    ...
    live_pass = bool(
        live.get("V2_ARTIFACT_VERIFICATION_PASS") is True
        and recorded == live
    )
    passed = bool(
...
def _audit_frozen_protocol_for_formal(
    ...
    """Validate v2 protocol assets without importing any execution module."""

    from .synthetic_confirmatory_protocol import EXPECTED_HYPOTHESES
    from .synthetic_confirmatory_v2_contract import (
    ...
    gate_value = _load_json_object(root / manifest["gate_contract_path"], "v2 gate")
    gate_unsigned = {name: value for name, value in gate_value.items() if name != "gate_contract_payload_sha256"}
    gate_pass = bool(
        gate_value.get("schema_version") == GATE_SCHEMA
        and gate_value.get("hypotheses") == EXPECTED_HYPOTHESES
        and gate_value.get("hypothesis_count") == 6
        and gate_value.get("all_hypotheses_required") is True
        and gate_value.get("gate_contract_payload_sha256")
        == canonical_identity_sha256(gate_unsigned)
    )
    gate = {
        "CONFIRMATORY_GATE_CONTRACT_PASS": gate_pass,
        "recorded_gate_contract_payload_sha256": gate_value.get("gate_contract_payload_sha256"),
    }
    schedule = _load_json_object(root / manifest["seed_schedule_path"], "v2 seed schedule")
    schedule_unsigned = {name: value for name, value in schedule.items() if name != "seed_schedule_payload_sha256"}
    seed_pass = bool(
        ...
        and schedule.get("seed_schedule_payload_sha256")
        == canonical_identity_sha256(schedule_unsigned)
        and [derive_seed("geometry", index) for index in range(5)] == list(GEOMETRY_SEEDS)
        and [derive_seed("measurement", index) for index in range(3)] == list(MEASUREMENT_SEEDS)
        and derive_seed("bootstrap", 0) == BOOTSTRAP_SEED
    )
    ...
    protocol_pass = bool(
        ...
        and protocol.get("seed_namespace") == NAMESPACE
        and protocol.get("lineage_schema_version") == LINEAGE_SCHEMA
    ...
    return {
        ...
        "SCIENTIFIC_SURVIVAL_BINDING_PASS": protocol.get("scientific_survival_audit_pass") is True,
...
def load_synthetic_confirmatory_stack(
    ...
    if manifest_file != (root / MANIFEST_RELATIVE).resolve():
        raise ValueError("Synthetic Confirmatory v2 requires its exact v2 manifest")
    verified_root, manifest = verify_manifest(
        manifest_file, require_authorized=require_authorized
    )
    if verified_root != root:
        raise ValueError("v2 manifest repository root mismatch")
...
def dry_run_synthetic_confirmatory(
    *, manifest_path: str | Path, run_id: str, output_dir: str | Path, workers: int
    ...
    if stack["cache_root"].exists() or stack["lock_path"].exists():
        raise FileExistsError(
            "Confirmatory v2 dry-run requires absent formal snapshot cache and lock"
        )
    ...
    snapshot_ids = [row["planned_snapshot_id"] for row in stack["snapshots"]]
    trial_ids = [row["planned_trial_id"] for row in stack["trials"]]
    ...
    passed = bool(
        ...
        and not stack["cache_root"].exists()
        and not stack["lock_path"].exists()
    )
    return {
        ...
        "snapshot_cache_dir": str(stack["cache_root"]),
        "snapshot_cache_dir_created": False,
        "snapshot_lock_path": str(stack["lock_path"]),
...
def execute_synthetic_confirmatory(
    *,
    ...
    from .synthetic_confirmatory_v2_snapshot_builder import (
        prepare_v2_snapshots,
    )

    monitor = FullSyntheticSourceAccessMonitor()
    monitor.install()
    preparation = prepare_v2_snapshots(
        stack["root"],
        stack["cache_root"],
        stack["snapshots"],
