...
def _mentions_boreas_lidar(value: str, *, require_bin: bool) -> bool:
    ...
    # Commands frequently embed URLs in Python/shell string literals, so a
    # closing quote, comma, or parenthesis is also a valid object-key boundary.
    has_bin = (
        re.search(r"(?:^|[/])[^/?\s'\"(),;]+\.bin(?=[?&\s'\"(),;\]]|$)", normalized)
        is not None
    )
...
def command_attempts_boreas_lidar_download(command: Any) -> bool:
    ...
    normalized_argv_text = " ".join(tokens)
    inspection_text = f"{text} {normalized_argv_text}"
    ...
    if operation in AWS_PAYLOAD_OPERATIONS:
        ...
        return _mentions_boreas_lidar(inspection_text, require_bin=False)
    ...
    if "curl" in executables:
        return not head_only and _mentions_boreas_lidar(inspection_text, require_bin=True)
    if "wget" in executables:
        return "--spider" not in tokens and _mentions_boreas_lidar(
            inspection_text, require_bin=True
        )
...
class BoreasLidarPayloadGuard:
    ...
    def attestation(self) -> dict[str, Any]:
        """Report zero materialization within the explicitly monitored entrypoints.

        This is not a host-wide network monitor.  Formal audit evidence must
        additionally bind the producer source and compare local payload scans
        before and after the guarded operation.
        """
        ...
        value = {
            ...
            "monitoring_scope": [
                ...
                "requests.sessions.Session.request_when_requests_is_importable",
            ],
            "host_wide_network_monitor": False,
