#!/usr/bin/env python3
"""Run the single frozen Synthetic Confirmatory v1 manifest."""
...
def _assert_preimport_isolation() -> None:
    if os.environ.get("PYTHONNOUSERSITE") != "1":
        raise PermissionError("Confirmatory scripts require PYTHONNOUSERSITE=1")
    if os.environ.get("MAMBA_ROOT_PREFIX") != FROZEN_MAMBA_ROOT_PREFIX:
        raise PermissionError(
            f"Confirmatory scripts require MAMBA_ROOT_PREFIX={FROZEN_MAMBA_ROOT_PREFIX}"
        )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run or dry-run the frozen Synthetic Confirmatory v1 matrix"
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.dry_run:
        ...
        if not args.resume:
            parser.error("formal execution requires --resume")
