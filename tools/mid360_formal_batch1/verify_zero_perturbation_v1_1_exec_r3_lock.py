#!/usr/bin/env python3
"""Independently verify the explicit-provenance FMB1 Exec-R3 lock."""
...
from experiments.mid360_formal_batch1.zero_perturbation_v1_1_exec_r3_verify import (  # noqa: E402
    verify_exec_r3_lock,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--expected-lock-release-commit")
    ...
    lock_dir = (args.lock_dir or root / R3_RESULTS_DIR).expanduser().resolve(strict=True)
    report = verify_exec_r3_lock(
        root,
        lock_dir,
        expected_execution_code_commit=args.expected_execution_code_commit,
        expected_lock_release_commit=args.expected_lock_release_commit,
    )
