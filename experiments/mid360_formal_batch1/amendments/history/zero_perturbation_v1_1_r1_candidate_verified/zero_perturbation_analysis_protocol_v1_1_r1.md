...
This protocol is a candidate pending the independent activation verifier. It has no backend authority, does not activate the capture-radius track, and requires `actual_formal_trials=0` until a separate activation record and active-protocol pointer are authenticated.
