...
Candidate status: `CANDIDATE_PENDING_INDEPENDENT_ACTIVATION_VERIFIER`  
Amendment ID: `FMB1_ZERO_PERTURBATION_MAINLINE_V1_1_R1`  
Activation effective: `false`  
Formal trial count at this correction: `0`

This is the versioned pre-backend correction of the retained v1.1 proposal. It is not active merely because this file exists. Activation becomes effective only after the independent R1 protocol verifier, final-dataset reauthentication, formal-environment qualification, trial-plan/result-schema verification, a zero-trial activation record, and an authenticated `ACTIVE_PROTOCOL.json` pointer all pass. Until then, `FORMAL_AUTHORITY=false`, `FORMAL_ICP_UNLOCKED=false`, and `FORMAL_REGISTRATION_AUTHORIZED=false`.
...
## Candidate-to-active gate

This R1 text is intentionally a candidate in this phase. It confers no execution authority. A later activation record may make it effective only if independent verification passes while the formal trial count remains zero. Formal lock issuance still must leave `FORMAL_ICP_UNLOCKED=false` and `FORMAL_REGISTRATION_AUTHORIZED=false`; a separate future authorization is required before any real backend call.
