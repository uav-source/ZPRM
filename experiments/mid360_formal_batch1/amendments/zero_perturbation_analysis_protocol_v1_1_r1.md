# FMB1 Zero-Perturbation Analysis Protocol v1.1-R1-C1
...
The pre-lock clarification `FMB1_ZERO_PERTURBATION_MISSINGNESS_CLARIFICATION_V1_1_R1_C1` is part of this active protocol. It was frozen before formal lock and ICP at formal trial count zero.
...
## C1 deterministic missingness and nonfinite rule

All 360 planned trial IDs remain in accounting. Scientific nonfinite results are retained as authoritative scientific failures with null endpoint fields and explicit statuses; JSON NaN/Infinity tokens are forbidden, and scientific failures cannot be retried. Infrastructure attempts remain in lineage. The earliest schema-valid infrastructure-PASS scientific outcome is authoritative; if none exists, the trial remains an unresolved infrastructure failure in the planned denominator.

Every station/scene/backend summary reports planned N, authoritative scientific N, finite endpoint N, scientific undefined/nonfinite N, unresolved infrastructure N, resolved infrastructure-attempt N, solver-nonconverged finite N, endpoint-specific undefined N, and formal-summary status. Solver non-convergence with a finite endpoint remains a finite scientific value.

Formal station translation/rotation summaries require 10/10 finite endpoints and no unresolved/nonfinite outcomes. Formal scene summaries require 30/30 finite endpoints plus all three defined station summaries. If incomplete, formal median/q25/q75/q95 are null. Finite available-case summaries may be reported with N only as `AVAILABLE_CASE_DESCRIPTIVE_NOT_PRIMARY_NOT_INFERENTIAL`; they never replace formal summaries or enter inference. No imputation or winsorization is allowed.

The exact Weak/Rich permutation runs only with all six defined scene summaries; otherwise the estimand and p-value are null. Formal cross-backend Spearman requires 6/6 paired scene summaries or 18/18 paired station summaries. Incomplete pairs yield null formal rho; constant input yields `SPEARMAN_UNDEFINED_CONSTANT_INPUT`. Available-pair correlations are descriptive only.

Reassociation metrics use endpoint-specific missingness. Formal results retain `common_association_valid`, the frozen invalid-reason enum, and bounded invalid detail; schema, runner, and verifier must preserve them before formal lock. Formal scene metrics require 30/30 finite values; scene association requires six complete scene pairs; registered within-scene centered association and its permutation require 30 complete pairs in each of six scenes, 180 total. A missing common metric does not invalidate a finite pose endpoint.

Formal station systematic fraction requires all ten finite translation vectors. An incomplete station has null fraction and remains counted. If all ten finite vectors are exactly zero, the fraction remains defined as 0. A scene requires all three station fractions, and Weak/Rich comparison requires all six scene values.

The complete authoritative wording and status codes are frozen in `zero_perturbation_analysis_missingness_clarification_v1_1_r1_c1.json`.
