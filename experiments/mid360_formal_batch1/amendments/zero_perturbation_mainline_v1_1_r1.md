...
Before formal lock, R1-C1 additionally froze deterministic missingness and nonfinite handling at formal trial count zero. All planned rows stay in accounting. Formal station and scene summaries use complete finite coverage (10/10 and 30/30); scientific nonfinite and unresolved infrastructure outcomes make the affected formal summary null rather than silently shrinking N. Available-case finite summaries are descriptive only. Exact permutation and formal Spearman run only with their complete frozen scene/pair sets. Systematic fraction requires all ten station vectors, and reassociation missingness is endpoint-specific. The authoritative status codes and retry/undefined rules are in `zero_perturbation_analysis_missingness_clarification_v1_1_r1_c1.json`.
