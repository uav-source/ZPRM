"""Independent, registration-free verification for W04 evidence."""
...
W04_STATION_KEYS = tuple(("FMB1_W04", station) for station in ("S01", "S02", "S03"))
...
def verify_w04_ros_evidence(
    acquisition: Mapping[str, Any],
    assets: Mapping[str, Any],
    config: Union[Mapping[str, Any], Path, str],
) -> dict[str, Any]:
    """Re-read all six bags, rebuild all assets, and compare exact bytes."""

    schema = "mid360_fmb1_w04_deep_ros_verification_v1"
    try:
        repository = Path(str(acquisition["repository"])).resolve(strict=True)
        observed = run_w04_acquisition(repository, config)
        ...
        if observed.get("W04_ACQUISITION_PASS") is not True:
            raise ValueError("independent W04 acquisition rerun did not PASS")
        ...
        target_by_key = _keyed_rows(
            targets, ("scene_id", "station_id"), "W04 target"
        )
        ...
        if set(target_by_key) != set(W04_STATION_KEYS):
            raise ValueError("target keys are not exact W04 S01-S03")
        if len(snapshot_by_key) != 30:
            raise ValueError("W04 snapshot count is not 30")
        config_payload = _load_config(config)
    except Exception as exc:
        return _failure(schema, f"{type(exc).__name__}: {exc}")

    station_results: list[dict[str, Any]] = []
    for scene_id, station_id in W04_STATION_KEYS:
...
def verify_w04_geometry_evidence(
    assets: Mapping[str, Any],
    geometry: Mapping[str, Any],
    config: Union[Mapping[str, Any], Path, str],
) -> dict[str, Any]:
    """Recompute all thirty initial-only rows and the formal scene median."""

    schema = "mid360_fmb1_w04_deep_geometry_verification_v1"
    try:
        ...
        expected_payload = _geometry_compare_payload(geometry)
        observed_payload = _geometry_compare_payload(observed)
        if type(expected_payload) is not type(observed_payload) or expected_payload != observed_payload:
        ...
        scene = scenes[0]
        if scene.get("scene_id") != "FMB1_W04":
            raise ValueError("geometry scene is not FMB1_W04")
        for counter in (
            "open3d_registration_call_count",
            "pcl_cli_invocation_count",
            "other_registration_process_count",
